<?php
    ...
    try{
        ...
        foreach ($records as $f){
            ...
            $BusinessPartner->AddressInformation->Address->Telephone->Number->SubscriberID = $f['phone'];
            $BusinessPartner->AddressInformation->Address->Telephone->Number->CountryCode = $f['country'];
            ...
            $Order->SalesOrder->BuyerID = "ANZST1718-".$f['id'];
            ...
            $Order->SalesOrder->Item->ScheduleLine->DateTime = $created_date."T00:00:01";
            ...
            $IndividualMaterial->TextCollection->Text = "Redeemed Product: ".$f['purchased_product'];
            $IndividualMaterial->ObjectWarranties->StartDate = $f['purchased_date'];
            $IndividualMaterial->InvolvedParties->ValidityStartDate = $created_date;
            $IndividualMaterial->BusinessData->PurchaseDate = $created_date;
            $IndividualMaterial->ERPIdentification->SerialID = $f['serialNo'];

            $xml->asXml("../xml/SoundTrack-AULS".$f['id']."-".$f['id'].".xml");

            echo "<p>";
            echo "SoundTrack".$f['id']."-".$f['id'].".xml generated. <br/>";
            echo "</p>";
        }
    }
    catch (Exception $e) {
